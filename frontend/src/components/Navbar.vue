<template>
  <div :class="[isHome ? 'pt-8' : 'py-8']" class="navbar mb-2 px-10 items-center bg-gradient-to-b from-[#FFEFEF] via-[#ffefef7e] to-transparent']">
    <template v-if="!isHome">
      ...
      <router-link :to="{ name: 'home-page' }" class="flex-none flex-nowrap mr-8 lg:flex space-x-2">
        ...
        <span class="flex-1 text-lg hidden lg:block"> Valentine<span class="font-bold">Wall</span> </span>
      </router-link>
      ...
      <client-only>
        <div v-if="$store.getters.isLoggedIn" class="dropdown dropdown-end dropdown-hover -mb-2">
          ...
          <ul tabindex="0" class="p-2 shadow-md menu dropdown-content bg-base-100 rounded-box w-52">
            ...
            <li class="text-black"><router-link :to="{ name: 'about-page' }">About</router-link></li>
            <li class="text-red-500"><a @click.prevent="" class="cursor-pointer" @click="$store.dispatch('logout')">Logout</a></li>
          </ul>
        </div>

...
export default {
  ...
  methods: {
    async submitIDForm(e: SubmitEvent) {
      ...
      const associatedIdField = e.target.children.namedItem('associated_id');
      if (!associatedIdField || !(associatedIdField instanceof HTMLInputElement)) return;
      ...
      try {
        const resp = await fetch(import.meta.env.VITE_BACKEND_URL + '/user/connect_id', {
          ...
          body: JSON.stringify({
            associated_id: associatedIdField.value,
          })
        });
        ...
        if (resp.status >= 200 && resp.status <= 299) {
          ...
          this.$store.commit('SET_USER_ASSOCIATED_ID', associatedIdField.value);
